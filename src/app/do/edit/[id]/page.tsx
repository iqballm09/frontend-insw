...
import CenteredProgress from "@/components/ProgressBar";
...
const Page = ({ params }: { params: { id: string } }) => {
  ...
  if (
    ...
    if (data && queryBank.data && queryDoc.data) {
      console.log("assign");
      console.log("con_id:", ctx.containerIdActive);
      ...
      ctx.setRequestDetailForm({
        ...
        doExpired: !!data.requestDetailForm.reqdoExp ? data.requestDetailForm.reqdoExp as string : undefined,
        ...
        vesselName: !!data?.requestDetailForm.vesselName ? data?.requestDetailForm.vesselName : undefined,
        voyageNumber: !!data?.requestDetailForm.voyageNumber ? data?.requestDetailForm.voyageNumber : undefined,
  ...
  return (
    <div className="my-4">
      <div className="flex justify-between items-center text-4xl font-bold border-b pb-4">
        <h1>
          Edit DO Request | {!ctx.isContainer ? "Non Container" : "Container"}
        </h1>
      </div>
