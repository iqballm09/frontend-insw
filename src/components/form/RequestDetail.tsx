...
const FormRequestDetail = () => {
  ...
  const isOpenBLDoc = () => {
    const isFileExist =
      (!!form.watch("ladingBillFile") && !!form.watch("urlBlFile")) ||
      (!!context.requestDetailForm?.ladingBillFile &&
        !!context.requestDetailForm.urlBlFile);
    if (
      form.watch("ladingBillType") == "1" ||
      context.requestDetailForm?.ladingBillType == "1"
    ) {
      return true;
    }
    if (isFileExist) {
  ...
  const readOnlyStyle = `read-only:opacity-70 read-only:cursor-not-allowed read-only:bg-gray-300 read-only:ring-0 read-only:outline-none read-only:focus:outline-none read-only:focus:ring-0`;
  const labelStyle = context.isShippingLineProcessing
    ...
    : "";
  const labelStyleSL =
    context.isShippingLineProcessing || isRejected
      ? `opacity-100 cursor-not-allowed ring-0 outline-none focus:outline-none focus:ring-0`
      : "";
  return (
    <>
      <div className="flex justify-center gap-20 py-5 border-b">
        <div
          ...
          onKeyDown={(e) =>
            handleKeyDown(e, () => context.handleSpecificForm(0))
          }
          role="button"
          tabIndex={0}
        >
              ...
              if (context.isContainer && isSLInputsExist) {
                const checkInputDepo = context.containerItems.map((item) => {
                  if (!item.depoName || !item.noTelp) {
                    toast.error(
                      `Depo data on container number ${item.containerNumber} must be filled!`
                    );
                    return false;
                  }
                  return true;
            ...
            handleKeyDown(e, async () => {
              ...
              if (context.isShippingLineProcessing) {
                ...
                if (context.isContainer && isSLInputsExist) {
                  ...
                  if (!checkInputDepo.includes(false)) {
                    context.handleSpecificForm(
                      context.containerIdActive ? 3 : 0
                    );
                  }
                } else {
                  if (isSLInputsExist) {
        ...
        <Form {...form}>
          ...
          >
                    ...
                    <FormItem className="col-start-1 col-span-4 ">
                      ...
                      <FormControl>
                        <Input
                          ...
                          placeholder={
                            context.userInfo?.profile.details.full_name
                          }
                          {...field}
                        />
                      </FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="col-start-1 col-span-full ">
                      ...
                      <FormControl>
                        <textarea
                          className="flex w-full rounded-md border border-input bg-gray-300 bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium file:text-white placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-70"
                          {...field}
                          placeholder={
                            context.userInfo?.organization.address.address +
                            ", " +
                            context.userInfo?.organization.address.city
                          }
                          rows={5}
                          disabled
                        ></textarea>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="col-start-1 col-span-4">
                  <FormLabel className={labelStyleSL}>
                    Shipping Line *
                  </FormLabel>
                  <Popover>
                        ...
                        <CommandGroup className="max-h-[200px] overflow-y-auto">
                          {querySl.isSuccess &&
                            querySl.data
                              .filter(
                                (item: any) =>
                                  item.kode === keywordSL.toUpperCase() ||
                                  item.uraian
                                    .toUpperCase()
                                    .includes(keywordSL.toUpperCase())
                              )
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1 flex flex-col col-span-3 pt-[10px]">
                  ...
                  <Popover
                    open={calendarOpenBL}
                    onOpenChange={setCalendarOpenBL}
                  >
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1 flex flex-col col-span-4 pt-[10px]">
                  ...
                  <Popover
                    open={calendarOpenBC}
                    onOpenChange={setCalendarOpenBC}
                  >
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-1 flex flex-col col-span-4 pt-[10px]">
                  ...
                  <Popover
                    open={calendarOpenReqdoExp}
                    onOpenChange={setCalendarOpenReqdoExp}
                  >
                    <PopoverTrigger asChild>
                      <FormControl>
                        <Button
              ...
              render={({ field }) => (
                <FormItem className="col-span-4">
                  ...
                  <Select
                    ...
                    onValueChange={field.onChange}
                    defaultValue={
                      field.value != "999" ? field.value : undefined
                    }
                  >
                    <FormControl>
                      <SelectTrigger>
            ...
            {["Released", "Rejected", "Processed"].includes(
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="flex-1 flex flex-col col-span-3 pt-[10px]">
                        ...
                        <Popover
                          open={calendarOpenDORelease}
                          onOpenChange={setCalendarOpenDORelease}
                        >
                          <PopoverTrigger asChild>
                            <FormControl>
                              <Button
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="flex-1 flex flex-col col-span-3 pt-[10px]">
                        ...
                        <Popover
                          open={calendarOpenDOExpired}
                          onOpenChange={setCalendarOpenDOExpired}
                        >
                          <PopoverTrigger asChild>
                            <FormControl>
                              <Button
            ...
            <div className="col-span-12 flex gap-4 justify-end">
              {!isDisabled && !context.isShippingLineProcessing && (
                <Button
                  type="submit"
              ...
              <Button
                ...
                type="button"
                className="flex gap-2"
                disabled={!context.containerIdActive}
              >
                Next <ChevronRight />
              </Button>
            </div>
